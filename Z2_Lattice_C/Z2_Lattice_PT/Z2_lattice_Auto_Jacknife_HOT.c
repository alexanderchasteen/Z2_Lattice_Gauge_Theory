        ...
        for (x[1]=0;x[1]<SIZE;x[1]++)
            for (x[2]=0;x[2]<SIZE;x[2]++)
                for (x[3]=0;x[3]<SIZE;x[3]++)
                    for (d=0;d<4;d++)
                        link[x[0]][x[1]][x[2]][x[3]][d] = (drand48() < 0.5) ? 1 : -1;
    return;
}
...
int main(){
    ...
    dbeta=.001;
    thermal_sweeps=100000;  
    autocorrelation_sweeps=10000;  
    measurment_sweeps=30000;  
    maxlag=8000;
    ...
    FILE *fp;
    char filename[100];
    sprintf(filename, "HOTLattice_Size%d_TS%d_MS%d,AS%d.csv", SIZE, thermal_sweeps, measurment_sweeps,autocorrelation_sweeps);
    fp=fopen(filename, "w");
   ...
   FILE *fp2;
    ...
    sprintf(filename2, "HOTSwendsen and Ferrenberg Sweeps_Lattice_Size%d_TS%d_MS%d,AS%d.csv", SIZE, thermal_sweeps, measurment_sweeps,autocorrelation_sweeps);
    ...
    sprintf(filename3, "HOTSusceptibility_Jacknife_Samples_Lattice_Size%d_TS%d_MS%d,AS%d.csv", SIZE, thermal_sweeps, measurment_sweeps,autocorrelation_sweeps);
    fp3 = fopen(filename3, "w");


    FILE *fp4;
    char filename4[100];
    sprintf(filename4, "HOTRaw_Monte_Carlo_Measurment%d_TS%d_MS%d,AS%d.csv", SIZE, thermal_sweeps, measurment_sweeps,autocorrelation_sweeps);
