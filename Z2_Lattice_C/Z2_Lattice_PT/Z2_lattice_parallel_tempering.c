...
#define SIZE 6
#define CONFIG 100    /* Number of parallel tempering configurations */
...
void over_relaxation_update(int lattice_config){
    int x[4],d,dperp,staple,staplesum;
    for (x[0]=0; x[0]<SIZE; x[0]++)
        for (x[1]=0; x[1]<SIZE; x[1]++)
            for (x[2]=0; x[2]<SIZE; x[2]++)
                for (x[3]=0; x[3]<SIZE; x[3]++)
                    for (d=0; d<4; d++) {
                        ...
                        if (staplesum != 0){
                            link[x[0]][x[1]][x[2]][x[3]][d][lattice_config] =(staplesum > 0 ? -1 : 1);
                        }
                    }                
}
...
int main(){
    ...
    int thermal_sweeps, measurment_sweeps, maxlag, autocorrelation_sweeps,blocks;
    int over_relaxation_param=0;
    ...
    char filename4[100];
    // 
    // sprintf(filename4, "PT%d_RawMC.csv", SIZE);
    sprintf(filename4, "PT_RENAME_RawMC.csv");
    fp4=fopen(filename4,"w");

     FILE *fp3;
    char filename3[100];
    sprintf(filename3, "PT_RENAME_RawTherm.csv");
    ...
    for (int j=0;j<thermal_sweeps;j++){
        ...
        if (counter % 10000==0){
            ...
            fprintf(fp,"Thermalization sweep %d out of %d \n",counter,thermal_sweeps);
        }

        for (int i=0; i<CONFIG;i++){
            double beta=beta_array[i];
            for (int j=0;j<over_relaxation_param;j++){
            over_relaxation_update(i);
            }
