...
SIZE=4
Nplaq=SIZE**4*6
df=pd.read_csv('PT4_RawMC (4).csv')
...
for i in range(CONFIGS):
    ...
    for k in np.linspace(-beta_nbhd_radius, beta_nbhd_radius, 1000):
        ...
        avg_plq_svendsen=np.mean(svendsen_blocking(avg_plq_svendsen_array,w_array,blocks))
        svendsen_plaq_array.append(avg_plq_svendsen)
        ...
        plaq_sus_svendsen=np.mean(svendsen_blocking(plaq_sus_svendsen_array,w_array,blocks))
        svendsen_sus_array.append(plaq_sus_svendsen)
