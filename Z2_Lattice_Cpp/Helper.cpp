...
std::array<int,4> sample_coord() {
    static std::uniform_int_distribution<int> dist1(0,3);
    static std::uniform_int_distribution<int> dist2(0,SIZE-1); 
    int x0=dist2(rng);
    int x1=dist2(rng);
    int x2=dist2(rng);
    int x3=dist2(rng);
...
double update_metropolis(std::array<int,array_size>& link, double beta, int lattice_config){
    std::array<int, 4> x; 
            ...
            for (x[2]=0; x[2]<SIZE; x[2]++)
                for (x[3]=0; x[3]<SIZE; x[3]++)
                    for (d=0; d<4; d++) {
                        ...
                        if (sample_uniform() < metropolis_number){
...
double update_heatbath(std::array<int,array_size>& link, double beta, int lattice_config){
    std::array<int, 4> x; 
...
void thermalize(std::array<int,array_size>& link, int therm_sweeps, double beta, int lattice_config){
    for (int i=0; i<therm_sweeps; i++){
        update_heatbath(link,beta,lattice_config);
    }
}
