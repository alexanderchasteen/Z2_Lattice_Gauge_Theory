...
int main() {
    ...
    // Code to initialize RNG in each thread. Need RNG for each thread that you need to seed.
    int n_threads = omp_get_max_threads();
    std::vector<std::mt19937> rng_threads(n_threads);
    for (int t = 0; t < n_threads; t++)
        rng_threads[t].seed(rng() + t);
    ...
    // Make the Necessary Arrays to keep track of the things
    std::array<double,CONFIG> beta_array=load_PT_Beta_Array("Checkpoint_Beta_array");
    std::array<int,CONFIG> beta_index_array=load_PT_index_Array("Checkpoint_Beta_index_array");
    std::array<int,array_size> links=load_lattice_config("Checkpoint_Lattice");
    ...
    for (int j=0;j<thermal_sweeps; j++){
        ...
        for (int i=0; i<CONFIG; i++){
            ...
            update_heatbath(links, beta, i, rng_threads[tid]);
            update_metropolis(links, beta, i, rng_threads[tid]);
            double action = update_metropolis(links, beta, i, rng_threads[tid]);
        ...
        for (int i=0; i<CONFIG-1;i++){
            ...
            double x = sample_uniform(rng_threads[0]); 
            if (x < Pswap) {
        ...
        Thermfile<<"\n";
        if (j % 10000 == 0) {  // every 10000 sweeps
            save_lattice_config(links, "Checkpoint_Lattice");
            save_PT_Beta_Array(beta_array, "Checkpoint_Beta_array");
            save_PT_index_Array(beta_index_array, "Checkpoint_Beta_index_array");
        }
    ...
    //         double beta=beta_array[i];
    //         double action=update_heatbath(links, beta,i,rng_threads[tid]);
    ...
    //         int tid = omp_get_thread_num();
    //         double beta=beta_array[i];
    //         thermalize(links, IAT_array[i]-1,beta,i,rng_threads[tid]);
    //         double action=update_heatbath(links,beta,i,rng_threads[tid]);
    //         action_array[i]=action;   
    //     }
